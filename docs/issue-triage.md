...
> `question`, `wontfix`) with **no** `type:`/`priority/`/`area:` scheme. If the
> repo later adopts a structured taxonomy, switch to the structured template.
>
> The commands below are GitHub Issues via the `gh` CLI. **The section headings
> must stay the same** — the skill and the `issue-triage-analyst` agent locate
> guidance by heading. Edit the prose under a heading, not the heading itself.
